...
const EmojiExplorer = () => {
	const randomEmojis = getEmojis(50).map(curr => {
		return {
			key: curr.title,
			data: curr
		}
	...
	const onChangeInput = (searchText: string) => {
		const filteredEmojis = filterEmoji(searchText, 50).map(curr => {
			return {
				key: curr.title,
				data: curr
			}
						...
						<div className="columns is-vcentered is-centered">
							<div className="column is-two-thirds content">
								...
								<div className="box">
									<input
										className="input"
										placeholder="Paste Emoji or Search e.g. Smile"
									...
									<table className="table" style={{ marginTop: 10 }}>
										<TransitionMotion
											...
											{styles => (
												<tbody>
													{styles.map(({ key, style, data: { symbol, title, keywords } }) => (
														<tr key={key} style={style}>
															<td>
																<div>
																	<figure>
																		<img
																			src={emojiImageSrc(symbol)}
																			height={24}
																			width={24}
																			alt={title}
																		/>
